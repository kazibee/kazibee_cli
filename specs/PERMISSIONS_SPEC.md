...
- Tool packages do not resolve `SYSTEM` vs `GLOBAL` vs `ENV` vs `LOCAL`.
...
A tool can declare a setup script that runs once at install/link time, before permission prompting. The setup script populates SYSTEM-level env values for the tool.

### Declaration

Add a `setup` field to the `kazibee` section in `package.json`:
...
The file must have a default export function. The function receives an empty `env: Record<string, string>` object. The function can set key-value pairs on the object, which are then persisted as SYSTEM-level env for the tool.
...
- Setup runs once at install/link time, before permission prompting.
- Values set by the setup script become SYSTEM-level env entries (lowest priority).
- SYSTEM values are overridable by GLOBAL, ENV, and LOCAL sources.
...
{
  "env": {
    "GEMINI_API_KEY": ["ENV:GEMINI_API_KEY", "LOCAL:GEMINI_API_KEY"],
...
1. String scoped:
   - `"SYSTEM:GEMINI_API_KEY"` (setup-time value)
   - `"GLOBAL:GEMINI_API_KEY"`
   - `"ENV:GEMINI_API_KEY"` (host environment)
...
- `SYSTEM`: values set by the tool's setup script during install/link time (lowest priority).
- `GLOBAL`: value from global persisted tool env scope.
- `ENV`: value from the host machine's `process.env`.
- `LOCAL`: value from directory-scoped persisted tool env scope (highest priority).
...
- `kazibee install ... --skip-permissions` and `kazibee link ... --skip-permissions` skip interactive prompts.
- When skipped, existing saved grants are left unchanged.
...
- Setup env stored in a dedicated `tool_setup_env` table for SYSTEM-level values set by setup scripts.
...
- No silent source substitution when a source is explicitly granted.
  - Example: if granted `ENV:GEMINI_API_KEY` but only local exists, do not inject.

## Candidate Resolution Rules

### Scoped candidate (`SYSTEM:KEY`, `GLOBAL:KEY`, `ENV:KEY`, `LOCAL:KEY`)
...
- Default recommended order: `LOCAL -> ENV -> GLOBAL -> SYSTEM`.
